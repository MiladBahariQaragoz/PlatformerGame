# Progress Log

Traceable record of curriculum tasks from [`plan.md`](./plan.md). Each completed task gets a
row with the commit that implemented it, so the game's history is auditable end to end.

**Status legend:** ☐ todo · ◐ in progress · ☑ done

## Totals

| Metric | Value |
| --- | --- |
| Tasks completed | 44 / 48 |
| XP earned | 345 / 360 |

## Module 1 — Your Game World

| Task | XP | Status | Commit |
| --- | --- | --- | --- |
| Set the Scene | 5 | ☑ | `d1db7e2` |
| Build the Path | 5 | ☑ | `7c9dacc` |
| What Makes a Game World | 10 | ☑ | `c284ba2` |
| Platforms and Level Layout | 10 | ☑ | `ecfbc35` |
| Add Your Character | 5 | ☑ | `7be00c2` |
| Characters and Identity | 10 | ☑ | `104bcbc` |
| Decorate the World | 5 | ☑ | `88e3ca0` |
| Details That Bring Worlds Alive | 10 | ☑ | `0113c04` |
| Set the Mood | 5 | ☑ | `a18f066` |
| Atmosphere and Tone | 10 | ☑ | `49f21dd` |
| World Building Recap | 10 | ☑ | `cdf7d44` |
| Module Quiz 1 | 10 | ☑ | `dbd794f` |

## Module 2 — Move and Jump

| Task | XP | Status | Commit |
| --- | --- | --- | --- |
| Make It Move | 5 | ☑ | `a490afc` |
| How Characters Move | 10 | ☑ | `1685d1b` |
| Add Jumping | 5 | ☑ | `b70af27` |
| The Art of Jumping | 10 | ☑ | `df52a01` |
| Extend the Level | 5 | ☑ | `65aed0f` |
| Collision and Solid Ground | 10 | ☑ | `abaada7` |
| Scroll the World | 5 | ☑ | `acf41f6` |
| Camera and Exploration | 10 | ☑ | `b549724` |
| Add Control Hints | 5 | ☑ | `2418ebe` |
| Game Feel and Polish | 10 | ☑ | `d8a688c` |
| Movement Recap | 10 | ☑ | `3277d59` |
| Module Quiz 2 | 10 | ☑ | `8ef1681` |

## Module 3 — Challenge and Reward

| Task | XP | Status | Commit |
| --- | --- | --- | --- |
| Add Collectibles | 5 | ☑ | `a363842` |
| Rewards and Motivation | 10 | ☑ | `979ca87` |
| Add an Enemy | 5 | ☑ | `5f69ea0` |
| Enemies and Obstacles | 10 | ☑ | `b92b8ac` |
| Add Hazards | 5 | ☑ | `07bc06d` |
| Difficulty and Fairness | 10 | ☑ | `597febe` |
| Add a Lives System | 5 | ☑ | `cf0a15f` |
| Stakes and Consequence | 10 | ☑ | `5ba7a05` |
| Add a Level Exit | 5 | ☑ | `2b6e99f` |
| Goals and Completion | 10 | ☑ | `17311d6` |
| Challenge Recap | 10 | ☑ | `9fa673a` |
| Module Quiz 3 | 10 | ☑ | `2492ea2` |

## Module 4 — Polish and Beyond

| Task | XP | Status | Commit |
| --- | --- | --- | --- |
| Add Juice Effects | 5 | ☑ | `a7e352c` |
| Juice and Feedback | 10 | ☑ | `3dfd7bc` |
| Add Sound Effects | 5 | ☑ | `c92e19d` |
| Sound Design | 10 | ☑ | `5f3402d` |
| Add a Start Screen | 5 | ☑ | `9d2fe99` |
| Menus and Flow | 10 | ☑ | `d0ad627` |
| Add a Game Over Screen | 5 | ☑ | `108bb1a` |
| Game Over and Retry | 10 | ☑ | `PENDING` |
| Final Creative Touches | 5 | ☐ | — |
| The Complete Picture | 10 | ☐ | — |
| Polish Recap | 10 | ☐ | — |
| Module Quiz 4 | 10 | ☐ | — |
